...
class NodeManager(Node):
    def __init__(self, node_file_dir=None, current_node=None):
        ...
        self.logger.info("NodeManager initialized with services")
    ...
    def path_planning_callback(self, request, response):
        ...
        try:
            ...
            if waypoints:
                ...
                self.logger.info(f"Path planning successful: {start_node} -> {target_node}, {len(waypoints)} waypoints")
                
                # 경로 시각화 (빨간색)
                self.visualize_path(waypoints, path_color=(1.0, 0.0, 1.0, 0.5))
            else:
                response.sum = -1  # 경로를 찾을 수 없음
                self.logger.warn(f"No path found: {start_node} -> {target_node}")
    ...
    def get_waypoints_callback(self, request, response):
        ...
        if not request.start_node and self.robot_position:
            ...
            if start_node is not None:
                self.logger.info(f"Auto-detected start node: {start_node} from robot position")
            else:
        ...
        try:
            ...
            if waypoints:
                ...
                self.logger.info(f"Waypoints generated successfully: {start_node} -> {goal_node}, {len(waypoints)} waypoints")
                ...
                response.node_path = []
                self.logger.warn(f"No path found: {start_node} -> {goal_node}")
        except Exception as e:
            response.success = False
            response.message = f"Failed to generate waypoints: {str(e)}"
            response.node_path = []
            self.logger.error(f"Waypoint generation failed: {str(e)}")
        return response
    ...
    def load_graph_callback(self, request, response):
        ...
        try:
            if self.node_list_path and os.path.exists(self.node_list_path):
                ...
                self.logger.warn("Graph file not found")
        except Exception as e:
            response.success = False
    ...
    def update_graph(self, path):
        if path and os.path.exists(path):
            ...
            self.logger.warn(f"Node list file not found: {path}")
    ...
    def make_dijkstra_path(self, start, target):
        try:
            ...
            self.logger.info(f"Dijkstra Path: {path}")
            self.logger.info(f"Dijkstra waypoint_list: {waypoint_list}")
    ...
    def get_waypoints_nodexy(self, shortest_path):
        ...
        for index in shortest_path:
            if index in self.node_graph.nodes:
                ...
                self.logger.warn(f"Index {index} not found in graph!")
    ...
    def make_ros_marker(self):
        self.set_link()
        self.set_node()
        self.logger.info("Marker Publishing Complete")
    ...
    def visualize_path(self, waypoints, path_color=(1.0, 1.0, 0.0, 1.0)):
        ...
        try:
            ...
            self.logger.error(f"Error in visualize_path: {str(e)}")
            self.clear_path_markers()
    ...
    def update_robot_position(self):
        ...
        try:
            ...
            if nearest_node != self.current_node:
                ...
                self.logger.info(f"Robot moved to node: {nearest_node}")
