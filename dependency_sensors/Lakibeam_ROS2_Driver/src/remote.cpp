...
int sensor_config(std::string sensor_ipaddr, std::string parameter, std::string value)
{
	...
	if(node_handle) {
		...
		printf("URL_RESTFUL_API%s\n",URL_RESTFUL_API.c_str());
	}
	...
	if(curl) {
		...
		if (curl_easy_perform(curl) == CURLE_OK){
			...
			if(http_code == 200)
			{
				if(node_handle) {
					...
					printf("Set %s, Value: %s ... done\n", URL_RESTFUL_API.c_str(), value.c_str());
				}
			}
			else
			{
		...
		else
		{
			if(node_handle) {
				...
				printf("http put error! please check lidar connection!\n");
			}
		}
	}
