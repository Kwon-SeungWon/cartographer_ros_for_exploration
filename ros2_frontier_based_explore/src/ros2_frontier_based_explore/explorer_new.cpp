...
namespace ros2_frontier_based_explore
{
  Explore::Explore() : Node("explore")
  {
    ...
    this->get_parameter_or<double>("gain_scale", gain_scale_, 0.01);
    ...
    this->get_parameter_or<int>("progress_timeout", timeout_, 10);
    ...
    unsigned int planner_period = 1000; // in milli secs
  ...
  void Explore::makePlan()
  {
    ...
    if (frontiers.empty())
    {
      ...
      {
        if(!state){
          RCLCPP_INFO(this->get_logger(), "11111111111111111111111111111111111");
    ...
    if (frontier == frontiers.end())
    {
      // stop();
      std_msgs::msg::Empty msgs;
      RCLCPP_INFO(this->get_logger(), "22222222222222222222222222222222222");
      finish_pub_->publish(msgs);
      backtoOrigin();
      return;
    }
    ...
    send_goal_options.result_callback = [this](auto result) {
        if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
            RCLCPP_INFO(this->get_logger(), "Reached succeeded!");
        } else {
            // RCLCPP_ERROR(this->get_logger(), "Reached failed!");
        }
    };
  ...
  void Explore::backtoOrigin(){

    std::lock_guard<std::mutex> lck(mutex_);
    
    RCLCPP_INFO(this->get_logger(), "backtoOrigin!!!");
    // send goal to nav2
    ClientT::Goal client_goal;
    client_goal.pose.pose.position.x = 0.0;
    client_goal.pose.pose.position.y = 0.0;
    client_goal.pose.pose.orientation.w = 1.;
    client_goal.pose.header.frame_id = "map";
    client_goal.pose.header.stamp = this->now();
  ...
  bool Explore::goalOnBlacklist(const geometry_msgs::msg::Point &goal)
  {
    ...
    constexpr static size_t tolerace = 40;
