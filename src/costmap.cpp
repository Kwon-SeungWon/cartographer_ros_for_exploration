...
namespace ros2_frontier_based_explore
{
  ...
  std::array<unsigned char, 256> init_translation_table2()
  {
    std::array<unsigned char, 256> cost_translation_table;
  ...
  static const std::array<unsigned char, 256> cost_translation_table2__ = init_translation_table2();

  void Costmap::updateFullMap(const nav_msgs::msg::OccupancyGrid::SharedPtr msg,bool state)
  {
    ...
    for (size_t i = 0; i < costmap_size && i < msg->data.size(); ++i)
    {
      ...
      if(state)
        ...
        costmap_data[i] = cost_translation_table2__[cell_cost];
