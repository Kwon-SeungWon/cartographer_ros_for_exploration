...
void Core::setParam()
{
    ...
    this->declare_parameter("can_mode", true);
    this->declare_parameter("Motor_driver", 0x00B7B801);
    this->declare_parameter("RobotName", "04");

    this->declare_parameter("Upper_id", 0x1A1B101);
    this->declare_parameter("Lower_id", 0x1B1A101);
...
void Core::getParam()
{
    ...
    this->get_parameter("Upper_id", Upper_id);
    this->get_parameter("Lower_id", Lower_id);
...
void Core::runCanModeControl()
{
    ...
    if(RobotName == "WheelChair") // if upper and rower level controller is combined
    {
    ...
    else // if controller is devided into two level
    { 
        uint8_t buf[8] = {0}; //declare and for safety

        int16_t cmd_vel_x = ctr_vel->linear.x * SIGNIFICANT_FIGURES;
        int16_t cmd_vel_z = ctr_vel->angular.z * SIGNIFICANT_FIGURES;

        buf[0] = (cmd_vel_x & 0xff);
        buf[1] = (cmd_vel_x >> 8) & 0xff;
        buf[4] = (cmd_vel_z & 0xff);
        buf[5] = (cmd_vel_z >> 8) & 0xff;

        bool send_result = can_->sendRawCan(Upper_id, buf);  // sendCan with out rpm calculate

        if (!send_result) {
                RCLCPP_ERROR(this->get_logger(), "[CAN] Failed to send CAN message");
            } else {
                RCLCPP_INFO(this->get_logger(), "[CAN] Sent CAN message: cmd_vel_x=%d, cmd_vel_z=%d", cmd_vel_x, cmd_vel_z);
            }
        }    
}
...
void Core::canFrameCallback(const can_frame& frame) {
    ...
    if(RobotName == "WheelChair"){
        ...
        if((frame.can_id & 0x1FFFFFFF) == static_cast<uint32_t>(Lower_id))
            {
                int16_t enc_vx, enc_w;
            
                enc_vx = static_cast<int16_t>(frame.data[0]) | (static_cast<int16_t>(frame.data[1]) << 8);
                enc_w  = static_cast<int16_t>(frame.data[4]) | (static_cast<int16_t>(frame.data[5]) << 8);
